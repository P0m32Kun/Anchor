...
|--------|------|----------|----------------------------|
| M0 | 🟡 进行中 | — | Tauri-Go 通信 + Scope Check + Subfinder 最小闭环 |
| M1 | ⚪ 待开始 | — | 目标输入 + Scope Check + 执行计划预览 |
...
验证最小闭环：能创建项目 → 添加目标 → 通过 Scope Check → 调一次 Subfinder → 保存 RawArtifact → Tauri UI 展示结果。
...
- [ ] **8. M0 最小闭环验收**
  - [ ] 8.1 创建项目（需联调验证）
  - [ ] 8.2 添加目标（域名）（需联调验证）
  - [ ] 8.3 通过 Scope Check（需联调验证）
  - [ ] 8.4 调一次 Subfinder（需联调验证）
  - [ ] 8.5 保存 RawArtifact（需联调验证）
  - [ ] 8.6 Tauri UI 展示结果（需联调验证）
