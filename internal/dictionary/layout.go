...
// Layout describes the on-disk directory tree rooted at {dataDir}/dictionaries.
type Layout struct {
	Root string
}

func NewLayout(dataDir string) Layout {
	return Layout{Root: filepath.Join(dataDir, "dictionaries")}
}
...
func (l Layout) FilePath(id string) string {
	return filepath.Join(l.Root, id+".txt")
}

func (l Layout) WriteFile(id string, data []byte) error {
	path := l.FilePath(id)
	if err := safefs.ValidateRelPath(id + ".txt"); err != nil {
