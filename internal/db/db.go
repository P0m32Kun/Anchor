...
func migrate(db *sql.DB) error {
	...
	if version < 30 {
		if err := migrateV30(db); err != nil {
			return fmt.Errorf("migrate v30 (bounty_candidates): %w", err)
		}
		if _, err := db.Exec("PRAGMA user_version = 30"); err != nil {
			return fmt.Errorf("set user_version 30: %w", err)
		}
		version = 30
	}
	if version < 31 {
		if err := migrateV31(db); err != nil {
			return fmt.Errorf("migrate v31 (submission_packs): %w", err)
		}
		if _, err := db.Exec("PRAGMA user_version = 31"); err != nil {
			return fmt.Errorf("set user_version 31: %w", err)
		}
		version = 31
	}
	if version < 32 {
		if err := migrateV32(db); err != nil {
			return fmt.Errorf("migrate v32 (tool_call_logs): %w", err)
		}
		if _, err := db.Exec("PRAGMA user_version = 32"); err != nil {
			return fmt.Errorf("set user_version 32: %w", err)
		}
		version = 32
	}
	if version < 33 {
	...
	if version < 45 {
		if err := migrateV45(db); err != nil {
			return fmt.Errorf("migrate v45 (assets cidr check): %w", err)
		}
