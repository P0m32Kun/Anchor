...
| v0.4 智能扫描管线                | 🔄 开发中 | —           | 多目标类型（company）+ FOFA 集成 + CDN 过滤 + nerva 服务指纹 + dnsx DNS 解析 + Pipeline 编排 + Hunter/Quake 搜索引擎 + Nuclei 分层扫描（workflow/tags/both）+ RBKD-SEC/templates + 速率防爆破（-rlm/-c）（功能已实现，待正式发布） |
